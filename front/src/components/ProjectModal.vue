...
<template>
  <BaseModal v-if="isOpen" width="!w-[46rem]" tabindex="0" actions-push-left="mr-6" :is-default-actions="true"
    role="dialog" ref="dialog" key="EDIT_PROJECT_DIALOG">
    <template #title>
    ...
    <template #content>
      <form class="project-form container">
        ...
        <div class="form-group">
          <BaseInput :value="props.project.requirements" class="requirements" field-name="requirements"
            label="Requirements" data-cy="edit-project-requirements"
            @input="updateProject('requirements', $event.target.value)" />
        </div>

        <div class="form-group">
          <BaseInput :value="props.project.milestones" class="milestones" field-name="milestones" label="Milestones"
            data-cy="edit-project-milestones" @input="updateProject('milestones', $event.target.value)" />
        </div>
        ...
        <div class="form-group">
          <BaseSelect :initial-selection="props.project.paymentMethod" :list="PAYMENT_METHODS" class="payment-method" field-name="payment-method"
            label="Payment Method" data-cy="edit-project-payment-method"
            @selected-tags="updateProject('paymentMethod', $event)" />
        </div>

        <div class="form-group budget">
          <BaseInput :value="props.project.budget" field-name="budget" label="Budget" data-cy="edit-project-budget"
        ...
        <div class="form-group form-group-status">
          ...
          <BaseToggle :is-checked="props.project.hasDownPayment" class="has-down-payment" field-name="has-down-payment"
            label="Has Down Payment" data-cy="edit-project-has-down-payment"
            @toggled="updateProject('hasDownPayment', $event)" />

          <BaseToggle :is-checked="props.project.wasPaymentDone" class="was-payment-done" field-name="was-payment-done"
            label="Was Payment Done" data-cy="edit-project-was-payment-done"
            @toggled="updateProject('wasPaymentDone', $event)" />

          <BaseToggle :is-checked="props.project.isCompleted" class="is-completed" field-name="is-completed"
            label="Is Completed" data-cy="edit-project-is-completed"
            @toggled="updateProject('isCompleted', $event)" />

          <BaseToggle :is-checked="props.project.isInvoiceSent" class="is-invoice-sent" field-name="is-invoice-sent"
            label="Is Invoice Sent" data-cy="edit-project-is-invoice-sent"
            @toggled="updateProject('isInvoiceSent', $event)" />

            <BaseToggle :is-checked="props.project.wasPaidInFull" class="was-paid-in-full" field-name="was-paid-in-full"
            label="Is Invoice Sent" data-cy="edit-project-was-paid-in-full"
